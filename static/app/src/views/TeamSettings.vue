...
<h1 id="teamName">{{ team.name }}</h1>
  <div class="card mb-2">
   ...
   <div v-if="team.jlt" class="card-body">
   	<a :href="'/api/v1/team/' + team.id + '/join/' + team.jlt">copy  this link</a> to share with agents
  ...
  <div class="card-body">
    <div><label>Rename Team <input v-on:change="renameTeam" v-model="teamName"></label></div>
    <div><hr/><label>Delete this team </label><button v-on:click="deleteTeam">Delete</button></div>
  </div>
 </div>
 ...
 <div class="card mb-2">
  <div class="card-header">enlightened.rocks Integration</div>
   <div class="card-body">
     <div>
     	Rocks Community Identifier: <input type="text" v-model="team.rc" placeholder="afdviaren.com" v-on:change="updateRocks"/> <span class="dim small">Typically looks like "randomstring.com"</span>
     </div>
     <div>
     	Rocks Community API Key: <input type="text" v-model="team.rk" placeholder="VnNfDerpL1nKsppMerZvwaXX" v-on:change="updateRocks"/> <span class="dim small">24 letter string</span>
     </div>
    ...
    <button v-on:click="pullRocks">Pull associated enl.rocks community members onto this team</button>
   </div>
  </div>
  <div class="card mb-2">
   <div class="card-header">V integration</div>
    <div class="card-body">
      <div>V Team ID #:
      	<input type="text" v-model="team.vt" v-on:change="updateV" placeholder="1234"/>
      </div>
      <div>V Team Role:
    	...
    	<button v-on:click="pullV">Pull associated V team/role members onto this team</button>
   </div>
  </div>
 <div class="card mb-2">
 ...
 <div class="card mb-2">
  <div class="card-header">Change Ownership</div>
   <div class="card-body">
    <input type="text" placeholder="new owner" v-model="newOwner" v-on:change="changeOwner"></input>
