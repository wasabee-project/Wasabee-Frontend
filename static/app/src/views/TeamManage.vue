...
<div class="container">
	<div class="row">
		<div class="col">
			...
			<input type="text" placeholder="GoogleID or Agent Name" v-model="agentName"/>
			...
			<tbody id="teamTable">
				<tr v-for="agent in team.agents" :key="agent.id">
					<td><img :src="agent.pic" height="50" width="50"></td>
						<td>{{ agent.name }}</td>
						<td><img v-if="agent.state" :src="window.wasabeewebui.cdnurl+'/mg/checkmark.png'" alt="sharing location"></td>
						<td><input v-model="agent.squad" v-on:change="agentSquadChange(agent)"/></td>
						<td><button v-if="!team.RockCommunity" v-on:click="removeAgent(agent)">Remove</button></td>
					</tr>
				</tbody>
			</table>
		</div>
